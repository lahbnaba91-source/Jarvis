  ...
  function soundUpdate() {
    ...
    if (want && !playing) {
      ...
      audio.play().catch(err => {
        playing = false;
        console.error("[ai-visualizer] thinking sound blocked — call AV.unlockSound() "
          + "from inside a real click handler once per page load", err);
      });
    } else if (!want && playing) {
      stopSound();
    }
  }
  // This player is driven from the /state poll loop, not a click — so its
  // first play() has no user-gesture backing it and browsers block that.
  // Call this once from inside any real click handler (a page that embeds
  // this face and adds its own interactive controls, e.g. the voice line's
  // chip click, should do so) to unlock it for every automatic play after.
  A.unlockSound = () => {
    if (!audio) return;
