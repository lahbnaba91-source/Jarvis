...
- **Multi-session safety.** The `scripts/session/guard.sh` SessionStart hook reports other live Jarvis sessions. If it warns that another session shares this working tree: do **not** run any branch switch, rebase, merge, or history rewrite in it, and don't commit to the same branch the other session is on. Isolate first — `scripts/session/worktree.sh <name>` makes a private worktree + branch — or agree out loud which session owns the tree while the other holds off. Two sessions on one tree collide on checkouts, stashes and refs; this cost real recovery work on 2026-09-04.
