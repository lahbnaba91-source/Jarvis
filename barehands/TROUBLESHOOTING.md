...
- **Choppy / low fps on a slow machine**: capture already defaults to `1280x720` (2026-09-04) since MediaPipe downscales every frame to its model input regardless, so this costs zero tracking accuracy. If a machine still struggles, `?res=` still overrides both ways — nothing lower is pre-tuned, but the knob goes up too: `?res=1920x1080` or `?res=3840x2160` for a sharper self-view on machines that can carry it.
